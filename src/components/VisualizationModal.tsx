...
export const VisualizationModal = ({ automaton, open, onClose }: VisualizationModalProps) => {
  ...
  const createSimpleSVG = (fa: FiniteAutomaton): string => {
    ...
    let svg = `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">`;
    svg += '<defs><marker id="arrowhead" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto" markerUnits="strokeWidth"><polygon points="0 0, 10 3.5, 0 7" fill="#333" /></marker></defs>';
    ...
    fa.transitions.forEach(transition => {
      ...
      if (fromPos && toPos) {
        ...
        if (transition.from === transition.to) {
          const loopRadius = 60;
          const startAngle = -Math.PI / 1; // -90 degrees
          const endAngle = Math.PI / 15;    // 60 degrees
          
          const startX = fromPos.x + Math.cos(startAngle) * stateRadius;
          const startY = fromPos.y + Math.sin(startAngle) * stateRadius;
          const endX = fromPos.x + Math.cos(endAngle) * stateRadius;
          const endY = fromPos.y + Math.sin(endAngle) * stateRadius;
          
          const controlX = fromPos.x;
          const controlY = fromPos.y - stateRadius - loopRadius;
          
          svg += `<path d="M ${startX} ${startY} Q ${controlX} ${controlY} ${endX} ${endY}" stroke="#333" fill="none" marker-end="url(#arrowhead)" stroke-width="2" />`;
          svg += `<text x="${controlX}" y="${controlY - 8}" text-anchor="middle" font-size="12" fill="#333">${transition.symbol}</text>`;
        } else {
          // Calculate edge points on circle circumference
          const dx = toPos.x - fromPos.x;
          const dy = toPos.y - fromPos.y;
    ...
    if (startPos) {
      ...
      const endArrowX = startPos.x - stateRadius;
      svg += `<line x1="${startArrowX}" y1="${startPos.y}" x2="${endArrowX}" y2="${startPos.y}" stroke="#333" marker-end="url(#arrowhead)" stroke-width="2" />`;
      svg += `<text x="${startArrowX - 10}" y="${startPos.y + 4}" text-anchor="end" font-size="12" fill="#333">start</text>`;
    }
